...
@app.get('/v1/admin/timecard/download', response_model=AdminFileDownloadResponseModel)
def get_timecard_download_api(credentials: HTTPAuthorizationCredentials = Security(security)):
    ...
    if (auth_handler.decode_token(token)):
        return FileResponse(file_name, media_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', headers={
...
#get users data from database for admin view only AdminTimesheetResponseModelAllUsers download as xlxs file
@app.get('/v1/admin/timecard/download', response_model=AdminFileDownloadResponseModel)
def get_timecard_download_api():

    """
    This timecard API allows you to fetch a report of user time data
    """

    # get admin data from database
    user = admin_get_all_users_timesheet_download()
    ...
    return FileResponse(file_path, media_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', headers={
            'Content-Disposition': f'attachment; filename="{file_name}"'
        })
